...
const CreateNewUserType = props => {
  ...
  // handle check all modules on Modules tab
  const createCheckAllModules = () => {
    let checkAllModules = [];
    for (let [i, item] of moduleFields.entries()) {
      let fields = [];
      for (let [j, field] of item.fields.entries()) {
        let subItem = {
          label: field,
          isChecked: false,
        };
  ...
  const [checkAllModuleGroup, setCheckAllModuleGroup] = useState(createCheckAllModules());

  const context = useContext(AppContext)
  const {setTitle} = context

  const set2ndColumnHeight = () => {
    const col1 = document.getElementById('moduleGroupCol1');
    const col2 = document.getElementById(`moduleGroupCol2-${openModuleGroup}`);
    let col1Height = null;
    let col2Height = null;
    if (col1 && col2) {
    ...
    if (col2Height > col1Height) {
      setModulesCol2Height(`${col2Height}px`);
    } else {
            ...
            <FormGroup row>
              <Col sm={12}>
                ...
                <TabContent activeTab={activeTab}>
                  ...
                  <TabPane tabId="2">
                    <Row>
                      <Col id="moduleGroupCol1" className="sc-col-border pt-3" lg={5} xs={12}>
                        {moduleFields.map((field, index) => {
                          return (
                            <ModuleGroup
                              ...
                              checkAll={checkAllModuleGroup}
                              setCheckAll={setCheckAllModuleGroup}
                            />
                          )
                        })}
                      </Col>
